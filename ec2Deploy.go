...
func deployRegionEC2(imageID string, count int64, region string, config Config) ([]CloudInstance, int) {
	...
	for _, instance := range runResult.Instances {
		ec2CloudInstance.ID = aws.StringValue(instance.InstanceId)
		ec2CloudInstance.IPv4 = aws.StringValue(instance.PublicIpAddress)
		ec2CloudInstance.Type = "EC2"
		ec2CloudInstance.Username = "ubuntu"
		ec2CloudInstance.SOCKSActive = false
		ec2CloudInstance.SOCKSPort = "0"
		ec2CloudInstance.Region = region
		ec2CloudInstance.PrivateKey = strings.Join(privKey[:len(privKey)-1], ".")
		ec2CloudInstance.Config = config
	...
	for _, region := range regionList {
		for _, instance := range ec2CloudInstances {
			if instance.Region == region {
				tempArray = append(tempArray, instance.ID)
			}
		}
...
func (instance *CloudInstance) describeInstanceSecurityGroup() *ec2.DescribeSecurityGroupsOutput {
	svc := createEC2Session(instance.Region, instance.Config.AWS.Secret, instance.Config.AWS.AccessID)
	securityGroups, err := svc.DescribeSecurityGroups(&ec2.DescribeSecurityGroupsInput{
		GroupIds: aws.StringSlice(instance.Firewalls),
	})
	if err != nil {
		log.Println("Error describing security group for AWS Instance: "  + instance.ID)
	}
	return securityGroups
}
...
func (instance *CloudInstance) setSecurityGroup(securityGroups []string) {
	...
	if err != nil {
		...
		instance.Firewalls = securityGroups
